 ...
 *
 */
final class ModelAggregatorProcessor2 implements Processor {

	private static final long serialVersionUID = -1685875718300564886L;
	private static final Logger logger = LoggerFactory.getLogger(ModelAggregatorProcessor2.class);
	...
	private ModelAggregatorProcessor2(Builder builder){
	...
	public boolean process(ContentEvent event) {
		...
		if(event instanceof InstanceContentEvent){
			...
			if (splittingNodeInfo != null) { // if null, that means
				...
				ActiveLearningNode2 activeLearningNode = splittingNodeInfo.activeLearningNode;
	...
	public Processor newProcessor(Processor p) {
		ModelAggregatorProcessor2 oldProcessor = (ModelAggregatorProcessor2)p;
		ModelAggregatorProcessor2 newProcessor =
				new ModelAggregatorProcessor2.Builder(oldProcessor).build();
	...
	private void trainOnInstanceImpl(Instance inst) {
		...
		if(leafNode instanceof LearningNode){
			...
			if(this.growthAllowed && (learningNode instanceof ActiveLearningNode2)){
				ActiveLearningNode2 activeLearningNode = (ActiveLearningNode2) learningNode;
	 ...
	 * tree model.
	 */
	private void attemptToSplit(ActiveLearningNode2 activeLearningNode, FoundNode foundNode){
	 ...
	 * tree model.
	 */
	private void continueAttemptToSplit(ActiveLearningNode2 activeLearningNode, FoundNode foundNode){
		...
		ActiveLearningNode2 node = activeLearningNode;
	 ...
	 */
	private void deactivateLearningNode(ActiveLearningNode2 toDeactivate, SplitNode parent, int parentBranch){
	...
	private LearningNode newLearningNode(double[] initialClassObservations, int parallelismHint){
		...
		return new ActiveLearningNode2(initialClassObservations, parallelismHint);
	}
	...
	static class SplittingNodeInfo{
		...
		private final ActiveLearningNode2 activeLearningNode;
		private final FoundNode foundNode;
		private final ScheduledFuture<?> scheduledFuture;

		SplittingNodeInfo(ActiveLearningNode2 activeLearningNode, FoundNode foundNode, ScheduledFuture<?> scheduledFuture){
	...
	static class Builder{
		...
		Builder(ModelAggregatorProcessor2 oldProcessor){
		...
		ModelAggregatorProcessor2 build(){
			return new ModelAggregatorProcessor2(this);
		}
	}
